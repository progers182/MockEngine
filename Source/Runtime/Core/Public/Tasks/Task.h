...
#define UE_SOURCE_LOCATION "UNIT_TEST"
...
bool Launch(std::string DebugName, TaskBodyType&& TaskBody) {
	TaskBody();
	return true;
}

template<typename TaskBodyType>
bool Launch(std::string DebugName, TaskBodyType&& TaskBody, bool Prerequisites) {
	if (Prerequisites) {
		return Launch(DebugName, TaskBody);
	}
	return false;
}
